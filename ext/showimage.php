...
Кожен раз при доступі до файлу в кеші в нього оновлюється час зміни.
Якщо файл не використовувався більше $cache_time, то він видаляється з
кешу. Якщо оригінальний файл змінився (розмір, час і т.п.), то кеш
автоматично оновлюється.
Перевірка виконується не кожен раз, а по рендому - 1 з 50 (в кінці файлу).
З'явився новий аргумент - background - колір фону (задається як в
хтмл, тільки без #, наприклад, background=FFFFFF). Потрібен якщо
картинки з прозорістю - тоді прозорі ділянки в них будуть не чорні, а
зафарбовані цим кольором.
