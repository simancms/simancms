...
function database_db_query($nameDB/*DEPRECATED*/, $sql, $lnkDB)
	{
		global $serverDB, $special, $_settings;
...
//Наступні функції застосовуються для спрощення, але при роботі з декількома БД бажано їх не використовувати
//Next function used for simplyfy but some of them can works unpropertly with multiple database access
...
function deletesql($tablename, $idname, $idval)
	{
		execsql("DELETE FROM $tablename WHERE $idname = $idval");
	}
...
function getsqlarray($sql, $type='a')
	{
		...
		if ($type=='r')
			{
				while ($row=database_fetch_row($result)) { $r[$i]=$row; $i++;}
			}
		elseif ($type=='o')
			{
				while ($row=database_fetch_object($result)) { $r[$i]=$row; $i++;}
			}
		else
			{
				while ($row=database_fetch_array($result)) { $r[$i]=$row; $i++;}
			}
		return $r;
	}
