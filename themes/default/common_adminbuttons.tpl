{strip}<div class="adminbuttons{if $bar.class neq ""} {$bar.class}{/if}"{if $bar.style neq ""} style="{$bar.style}"{/if}>{if $bar.buttonbar_title neq ""}<span class="buttonbar_title">{$bar.buttonbar_title}</span>{/if}
